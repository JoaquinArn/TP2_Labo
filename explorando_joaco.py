...
pseudo_por_pixel = {}
# for pixel in cartera.columns:
#     limite_inf_cartera = min(cartera[pixel] >= (cartera[pixel].quantile(0.25) - 1.5*iqr(cartera[pixel]))) #Calculamos máxima extensión del limite inferior que puede tener el whisker 
#     limite_sup_cartera = max(cartera[pixel] <= (cartera[pixel].quantile(0.75) + 1.5*iqr(cartera[pixel]))) #Calculamos máxima extensión del limite superior que puede tener el whisker 
#     pseudo_por_pixel[pixel] = abs(limite_sup_cartera - limite_inf_cartera)
# df_pseudo_cartera = pd.DataFrame([pseudo_por_pixel])
for pixel in cartera.columns:
    umbral_inf_cartera = cartera[pixel].quantile(0.25) - 1.5 * iqr(cartera[pixel])
    umbral_sup_cartera = cartera[pixel].quantile(0.75) + 1.5 * iqr(cartera[pixel])

    # Seleccionamos los valores reales dentro del rango permitido
    limite_inf_cartera = cartera.loc[cartera[pixel] >= umbral_inf_cartera, pixel].max()
    limite_sup_cartera = cartera.loc[cartera[pixel] <= umbral_sup_cartera, pixel].min()
    pseudo_por_pixel[pixel] = abs(limite_sup_cartera - limite_inf_cartera)
df_pseudo_cartera = pd.DataFrame([pseudo_por_pixel])
...
cartera = fashion[fashion['label'] == 8].drop(columns=["label"])
...
pseudo_por_pixel = {}
for pixel in cartera.columns:
    limite_inf_cartera = min(zapatilla[pixel] >= (zapatilla[pixel].quantile(0.25) - 1.5*iqr(zapatilla[pixel]))) #Calculamos máxima extensión del limite inferior que puede tener el whisker 
    limite_sup_cartera = max(zapatilla[pixel] <= (zapatilla[pixel].quantile(0.75) + 1.5*iqr(zapatilla[pixel]))) #Calculamos máxima extensión del limite superior que puede tener el whisker 
    pseudo_por_pixel[pixel] = abs(limite_sup_cartera - limite_inf_cartera)
...
clasificadorPseudo_rango = KNeighborsClassifier(n_neighbors=20)
...
def generar_grafico_matriz(matriz, ax, titulo):
    ...
    ax.set_title(f"Accuracy Score - Selección por {titulo}") #Le agregamos título al gráfico.
...
generar_grafico_matriz(np.array(resultadosMean), axes[0, 0], "Promedio")
generar_grafico_matriz(np.array(resultadosMedian), axes[0, 1], "Mediana")
generar_grafico_matriz(np.array(resultadosIQR), axes[1, 0], "IQR")
generar_grafico_matriz(np.array(resultadosPseudoRango), axes[1, 1], "Pseudo-Rango")
...
#%% IDEA EJERCICIO 1A funcion devuelve pixeles inutiles version mejorada
...
def devuelve_pixeles_irrelevantes_iqr(pixeles, clase):
    ...
    if clase > 0:
        return devuelve_pixeles_irrelevantes_iqr(res, clase-1)
    else:
        return res
...
#%% PLOTEAMOS EL ÁRBOL DE DECISIÓN SELECCIONADO
...
alturas = [6,7,8,9,10]
#Utilizaremos 9 folds, donde cada uno contendrá 700 datos por cada clase (7000 datos en total)
nsplits = 4
...
atributos = X_dev.columns  # Suponiendo que `X_dev` tiene los nombres de las características

df_importancias = pd.DataFrame({'Atributo': atributos, 'Importancia': importancias})
