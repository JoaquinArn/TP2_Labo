...
del fig, axes, i, nombre, df, img, promedio
...
df_promedio_saco = saco.mean().to_frame().T
...
plt.title("Imagen promedio segun los promedios de cada clase")
plt.axis('off')
plt.show()
...
plt.title('IQR de cada píxel entre las 10 imágenes promedio de clase', fontsize=16)
...
#Borramos variables auxiliares usadas en el proceso
del img, heatmap, iqr_, q1, q3, pixel
...
mejor_arbol = tree.DecisionTreeClassifier(max_depth = 10, criterion='entropy')
...
#%% OPCIÓN ALTERNATIVA: ARBOL CON MENOS ATRIBUTOS
#%% OPCION ALTERNATIVA: SELECCIONAMOS MENOS ATRIBUTOS
#En primer lugar, separamos nuestros conjuntos en:
    #el que utiizaremos como desarrollo del modelo
...
#%% SELECCIÓN MEJOR ARBOL ALTERNATIVO
...
grid_search = GridSearchCV(arbol_decision, parametros, cv=4, scoring='accuracy', verbose=1)
